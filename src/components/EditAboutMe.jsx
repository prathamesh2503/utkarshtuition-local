...
const EditAboutMe = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      try {
        const res = await fetch("https://utkarshtuition-backend.vercel.app/teacher");
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("https://utkarshtuition-backend.vercel.app/teacher", {
      method: "Post",
      body: formData,
    });
    if (!response.ok) {
  ...
  const handleDelete = async () => {
    const responseDel = await fetch(
      `https://utkarshtuition-backend.vercel.app/teacher/${teacher.id}`,
      {
        method: "DELETE",
